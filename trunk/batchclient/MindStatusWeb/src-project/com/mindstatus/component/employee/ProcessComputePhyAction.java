...
public class ProcessComputePhyAction extends Action
{
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		DynaActionForm dynaForm = (DynaActionForm) form;
		Date birthday = (Date) dynaForm.get("birthday");
		Date nowDay = (Date) dynaForm.get("nowDay");
		PhysiologyCycle pc = null;
		if (nowDay != null)
		{
			pc = PhysiologyCycleUtils.computePhysiologyCycle(birthday, nowDay);
		}
		else
		{
			pc = PhysiologyCycleUtils.computePhysiologyCycle(birthday);
		}
		dynaForm.set("age", Integer.valueOf(PhysiologyCycleUtils.computeAgeByBirthday(birthday)));
		...
		this.saveChart(request, birthday, nowDay);
		return mapping.findForward(ApplicationConst.ACTION_FORWARD_SUCCESS);
	}

	private void saveChart(HttpServletRequest request, Date birthday, Date nowDay) throws Exception
	{
		if (nowDay == null)
		{
		...
		Date[] days = PhysiologyCycleUtils.getSeriesMothDays(nowDay);
		...
		String fileName = ServletUtilities.saveChartAsJPEG(chart, 850, 400, request.getSession());
