...
@Component
public class CastedEuropeVideoEngine extends AbstractEngine implements Engine {

	private static final String ACCESS_DETAIL_NAME = "CastedEuropeVideo";
	...
	private void searchPage(String pageUrl, Set<String> finishedPages, AccessDetail accessDetail) throws Exception {
		...
		List<String> albumUris = URICollectionFilter.valueOf(hrefs).removeDuplicate().selectContains("fid=1").list();
	...
	private void searchAlbum(String albumUri, AccessDetail accessDetail) throws Exception {
		Map<String, String> parameters = HtmlUtils.getParametersFromUrl(albumUri);
		String fileName = parameters.get("vid") + ".wmv";
		File downloadFile = new File(accessDetail.getSavePath(), fileName);
		if (downloadFile.exists() && !accessDetail.isReplaceExist()) {
			logger.info("ignore exist : {}", albumUri);
		} else {
			SearchStatus searchStatus = new SearchStatus();
			searchStatus.setLastSearchUrl(albumUri);
			fireDownloadEvent(accessDetail, searchStatus, fileName, downloadFile.getAbsolutePath(), albumUri);
		}
	}
